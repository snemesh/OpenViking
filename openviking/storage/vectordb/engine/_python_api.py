...
def _request_list_to_backend(items: Iterable[_RequestBase]) -> list[dict[str, Any]]:
    return [item.to_backend() if hasattr(item, "to_backend") else item for item in items]
...
def build_abi3_exports(backend: Any) -> dict[str, Any]:
    class IndexEngine:
        ...
        def search(self, req: SearchRequest) -> SearchResult:
            return SearchResult.from_backend(
                self._backend._index_engine_search(self._handle, req.to_backend())
            )
